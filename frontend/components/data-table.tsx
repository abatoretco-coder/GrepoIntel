...
export function DataTable({ rows, kind }: { rows: Row[]; kind: Kind }) {
  ...
  return <div className="table-wrap"><table className="intel-table"><thead><tr>{headers.map((header) => <th key={header}>{header}</th>)}</tr></thead><tbody>{rows.map((row, index) => {
    ...
    if (kind === "player") return <tr key={id || index}><td className="primary-cell">{String(row.name)}</td><td>{row.alliance_id?<Link className="table-link" href={`/alliance/${Number(row.alliance_id)}`}>{String(row.alliance_name ?? "Alliance")}</Link>:"Sans alliance"}</td><td>{number(row.points)}</td><td>#{number(row.rank)}</td><td>{number(row.cities_count)}</td><td><Link className="table-link" href={`/player/${id}`}>Analyser →</Link></td></tr>;
