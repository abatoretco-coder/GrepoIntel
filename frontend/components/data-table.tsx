...
export function DataTable({ rows, kind }: { rows: Row[]; kind: Kind }) {
  ...
  const headers = kind === "player" ? ["Joueur", "Points", "Rang", "Villes", "Combat", "Fiche"] : kind === "alliance" ? ["Alliance", "Points", "Rang", "Membres", "Villes", "Fiche"] : kind === "city" ? ["Ville", "Coordonnées", "Points", "État", "Fiche"] : kind === "threat" ? ["Joueur", "Distance", "Niveau", "Score", "Signaux", "Fiche"] : ["Ville", "Distance", "Points", "Niveau", "Signaux", "Fiche"];
  return <div className="table-wrap"><table className="intel-table"><thead><tr>{headers.map((header) => <th key={header}>{header}</th>)}</tr></thead><tbody>{rows.map((row, index) => {
    ...
    if (kind === "player") return <tr key={id || index}><td className="primary-cell">{String(row.name)}<span className="sub-cell">ID {id}</span></td><td>{number(row.points)}</td><td>#{number(row.rank)}</td><td>{number(row.cities_count)}</td><td>{number((Number(row.attack_points) || 0) + (Number(row.defense_points) || 0))}</td><td><Link className="table-link" href={`/player/${id}`}>Analyser →</Link></td></tr>;
    ...
    return <tr key={id || index}><td className="primary-cell">{String(row.name)}</td><td>{number(row.distance)} cases</td><td>{number(row.points)}</td><td><span className={`score ${scoreClass(row.score)}`}>{number(row.score)}</span></td><td><div className="reason-list">{(Array.isArray(row.reasons) ? row.reasons : []).slice(0, 2).map((reason) => <span className="reason" key={String(reason)}>{String(reason)}</span>)}</div></td><td><Link className="table-link" href={`/${entity}/${id}`}>Analyser →</Link></td></tr>;
  })}</tbody></table></div>;
}
