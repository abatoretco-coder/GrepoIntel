...
class PersonalEmpireSnapshot(Base):
    ...
    captured_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), index=True); source_type: Mapped[str] = mapped_column(String(32)); source_version: Mapped[int] = mapped_column(Integer); state_hash: Mapped[str] = mapped_column(String(64)); global_state: Mapped[dict] = mapped_column(JSON, default=dict)
...
class PersonalStatePairing(Base):
    __tablename__ = "personal_state_pairings"; __table_args__ = (UniqueConstraint("profile_id"),)
    id: Mapped[int] = mapped_column(primary_key=True); profile_id: Mapped[int] = mapped_column(ForeignKey("user_profiles.id"), index=True)
    token_hash: Mapped[str] = mapped_column(String(64)); created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
